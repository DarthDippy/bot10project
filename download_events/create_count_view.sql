...
CREATE OR REPLACE VIEW decode.events_with_keyword_counts AS 
 SELECT events.event_id,
    ...
    events.start_time,
    events.end_time,
    decode.count_instances(events.event_description, 'comfort food'::text) + decode.count_instances(events.event_name, 'comfort food'::text) + decode.count_instances(events.event_description, 'social support'::text) + decode.count_instances(events.event_name, 'social support'::text) + decode.count_instances(events.event_description, 'flowers'::text) + decode.count_instances(events.event_name, 'flowers'::text) + decode.count_instances(events.event_description, 'socialize'::text) + decode.count_instances(events.event_name, 'socialize'::text) + decode.count_instances(events.event_description, 'music'::text) + decode.count_instances(events.event_name, 'music'::text) + decode.count_instances(events.event_description, 'gym'::text) + decode.count_instances(events.event_name, 'gym'::text) + decode.count_instances(events.event_description, 'shopping'::text) + decode.count_instances(events.event_name, 'shopping'::text) + decode.count_instances(events.event_description, 'sports'::text) + decode.count_instances(events.event_name, 'sports'::text) + decode.count_instances(events.event_description, 'dance'::text) + decode.count_instances(events.event_name, 'dance'::text) + decode.count_instances(events.event_description, 'games'::text) + decode.count_instances(events.event_name, 'games'::text) AS negative_keyword_count,
